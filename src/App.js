...
function App() {
 ...
 /* useEffect(() => {
  ...
  const fetchImg = async (day='') => {
    console.log("https://api.nasa.gov/planetary/apod?api_key=PCFVBxbEyHAfHEySNWOuSHcgtuY7rhkrfrgi6ueX&date="+day)
    const res = await fetch("https://api.nasa.gov/planetary/apod?api_key=PCFVBxbEyHAfHEySNWOuSHcgtuY7rhkrfrgi6ueX&date="+day)
    const data = res.json()
    return data
  }
